...
> 状态：WP0–WP5 全部落地（2026-08-27，125 测试全绿）。**⚠️ 后续进度宣告以 `docs/GAPS.md` 为准**——实际使用发现与 Zed 终端体验仍有明显距离（用户确认痛点集中在应用骨架交互层），工作方式已从"工作包驱动"转为"缺口驱动"；本文件降级为决策记录与架构事实，不再逐项勾销。WP6 暂停至体验达标。
