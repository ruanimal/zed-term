...
/// The terminal program name and version reported to child processes.
...
impl TerminalBuilder {
    ...
    pub fn new(
        ...
        let version = TERMINAL_PROGRAM_VERSION;
        ...
        let fut = async move {
            ...
            insert_terminal_env(&mut env, version);
...
impl Terminal {
    ...
    fn register_task_finished(
        ...
        // For interactive shells we need to differentiate:
...
impl Drop for Terminal {
    fn drop(&mut self) {
        if let TerminalType::Pty { pty_tx, info } = &self.terminal_type {
            pty_tx.shutdown();
